...
for file in $(while read line; do echo $line | grep "example.com" | cut -f4- -d/ | cut -f1 -d\<; done < public_html/tmpsitemap.xml); do
        ...
        SITEMAP_ENTRY_A=$(cat public_html/tmpsitemap.xml | grep "example.com/${file}<"| sed 's/\//\\\//g' | sed 's/\./\\\./g' | sed 's/\-/\\\-/g')
        SITEMAP_ENTRY_B=$(cat public_html/tmpsitemap.xml | grep "example.com/${file}<" | sed "s/20[0-9]*-[0-9]*-[0-9]*/${DATE_FILE}/g" | sed 's/\//\\\//g' | sed 's/\./\\\./g' | sed 's/\-/\\\-/g')
        sed -i "s/${SITEMAP_ENTRY_A}/${SITEMAP_ENTRY_B}/g" public_html/tmpsitemap.xml && echo $file && echo "Old date: ${SITEMAP_ENTRY_A}" && echo "New date: ${SITEMAP_ENTRY_B}" && sleep 1
done
